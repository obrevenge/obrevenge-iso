...
iso_name=obrevenge
iso_label="obrevenge-$(date +%Y%m)-32bit"
...
# Build ISO
make_iso() {
    mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -o "${out_dir}" iso "${iso_name}-${iso_version}-32bit.iso"
}
...
if [[ ${arch} != i686 ]]; then
    echo "This script needs to be run on i686"
    _usage 1
fi
